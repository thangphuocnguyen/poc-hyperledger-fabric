...
dir=`basename $PWD`
if [ "${dir}" != "toolbox" ]; then
	echo "You should run this script from the toolbox directory."
	exit 1
fi
...
printHelp() {
  ...
  echo
  echo "e.g. bootstrap.sh 1.4.0 1.4.0"
  echo "would download and binaries for version 1.4.0 (fabric) 1.4.0 (fabric-ca)"
}
